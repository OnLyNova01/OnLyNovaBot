...
from userbot import CMD_HANDLER, CMD_LIST, DEFAULT, DEVS, NOVA2, NOVA3, NOVA4, RAM5, bot
...
def register(**args):
    ...
    def decorator(func):
        async def wrapper(check):
            ...
            try:
                ...
                if not disable_errors:
                    ...
                    text = "** OnLyNovaBot LAPORAN EROR  **\n\n"
                    link = "[ Support](https://t.me/OnLyNova)"
        ...
        if NOVA2:
            if not disable_edited:
                NOVA2.add_event_handler(wrapper, events.MessageEdited(**args))
            NOVA2.add_event_handler(wrapper, events.NewMessage(**args))
        if NOVA3:
            if not disable_edited:
                NOVA3.add_event_handler(wrapper, events.MessageEdited(**args))
            NOVA3.add_event_handler(wrapper, events.NewMessage(**args))
        if NOVA4:
            if not disable_edited:
                NOVA4.add_event_handler(wrapper, events.MessageEdited(**args))
            NOVA4.add_event_handler(wrapper, events.NewMessage(**args))
        if RAM5:
            if not disable_edited:
